...
stable tag so upstream updates can be merged and tested independently.
...
`.creator-signal/openobserve-version` records the exact upstream release merged into this branch.
The upstream watcher opens a PR for each newer stable tag. Nightly compatibility CI performs a
throw-away merge with upstream `main` to reveal refactor conflicts before the next stable release.

Release tags use `openobserve-v<upstream>-cs.<revision>`, for example:

```bash
git tag -a openobserve-v0.91.5-cs.1 -m "OpenObserve v0.91.5 with Creator Signal OIDC cs.1"
git push origin openobserve-v0.91.5-cs.1
```

The release workflow refuses a tag whose upstream version differs from the marker file or whose
commit does not descend from that upstream tag. It publishes a multi-architecture GHCR image,
keyless signature, registry provenance, SPDX SBOM, checksums, source archive, and validated
