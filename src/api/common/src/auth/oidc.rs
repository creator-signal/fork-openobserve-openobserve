...
use config::{
    ...
    utils::{json, rand::generate_random_string, str::is_valid_email},
};
use jsonwebtoken::{
...
use openobserve_core::{
    auth::{AuthExtractor, V2_API_PREFIX},
    kv, organization, users,
};
...
async fn read_binding(key: &str) -> anyhow::Result<Option<SubjectBinding>> {
    let keys = kv::list(OIDC_BINDING_NAMESPACE, key).await?;
    if !keys.iter().any(|candidate| candidate == key) {
        return Ok(None);
    }
    let value = kv::get(OIDC_BINDING_NAMESPACE, key).await?;
...
async fn bind_subject(identity: &Identity) -> anyhow::Result<()> {
    ...
    kv::set(OIDC_BINDING_NAMESPACE, &email_key, value.clone()).await?;
    kv::set(OIDC_BINDING_NAMESPACE, &subject_key, value).await?;
    Ok(())
}
